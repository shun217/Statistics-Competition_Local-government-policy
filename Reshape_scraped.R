...
# 1-1. データの読み込み
base <- read.csv("data/seijiyama_result.csv", fileEncoding = "UTF-8", stringsAsFactors = FALSE)
...
# 1-3. electedした人のみのデータに限定する
base <- base[base$elected == "True", ]

# 1-4. 選挙名から都道府県と市区町村を抽出（選挙名を後ろから見ていき、市区町村都道府県のワードが出たらそれより前の部分を取る）
name <- sub("（.*", "", base$選挙名)
